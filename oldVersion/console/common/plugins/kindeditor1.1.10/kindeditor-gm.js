KindEditor.plugin('diyupload', function(K) {  
	...
	editor.plugin.diyuploads = function(){
		...
		/**edit by zlm**/
		var thisGallery = $btn.attr("galleryInited", "true").gmgallery({touristLineId:editor.options.touristLineId,operateType:"other_image",options_id:new Date().getTime(),onOk:callback,zIndex:'900999'});
		$btn.click();
	}
	// 点击图标时执行
	...
	editor.plugin.recentlyuse_query_click = function(){
		...
		if(null != uri && "" != uri){
			if(editor.options.recentlyuseUrl.indexOf("?") >0){
				uri = editor.options.recentlyuseUrl + "&keyword=" + $("#typing_keyword").val();
			}else{
				uri = editor.options.recentlyuseUrl+"?keyword="+$("#typing_keyword").val();
			}
			$.ajax({
