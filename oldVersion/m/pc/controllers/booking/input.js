	...
	controller.modules = {
		...
		eCalendar: function() {
			...
			var id=$(".box-url").attr("data-url");
		    	$.getJSON("/item/"+id+"/prices",function(serverData){
		...
		questTime: function(time) {
			...
			$.ajax({
				url: '/item/' + $(".box-url").attr('data-url') + '/dates/' + dateTem + ".json",
				type: 'get',
				dataType: 'json',
				success: function(data) {
					if (data.itemDates) {
						$(".price_start").html('￥<em>' + data.itemDates.specifiedPrice + '</em>起/人');
						data.itemDates.extraPrice ? $(".extraPrice").html('(单房差：￥<em>' + data.itemDates.extraPrice + '</em>)') :$(".extraPrice").html("");
					} else {
