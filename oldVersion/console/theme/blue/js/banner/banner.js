$(function(){
	...
	for (var i = 0; i < familydata.length ;i++) {
		familydata[i].web_static = WEB_STATIC;		
		familyhtml += "<li data='{txt}' ><img src='{web_static}/console/theme/blue/images/fontImg/{img}'/></li>".template(familydata[i]);
	}
	...
	$dashboard.on("click", ".okText", function(){
		...
		//当没有勾选时，文本框不能点击
		if($(this).is(':checked')){
             $(".item").find("textarea.text").removeAttr("disabled");
		}else{
           $(".item").find("textarea.text").attr("disabled","disabled;");
		}
...
//					}
		...
		insertFont : function(data){
			var defaultfont = { text: "广告文案",zindex: 100, width:200, height:40, yaxis:0, xaxis:0, color : "#000000", size : "20px", bold: false, family: "微软雅黑" };
			...
			if(!$(".allowFont").find("input").is(':checked')){
                 $(".item").find("textarea.text").attr("disabled","disabled;");
			}
			$("[active!=show]",$element).hide();
			return $element;
	...
	webUpload({"target":"#insertImage","operateType":"user_image", success:function(response) {
						...
						if(xhr.readyState == 4) {
							if(xhr.status == 200) {
								...
								if(result.success) {
									...
									$jsondom.refresh();
									$.post('/banner/input.json',$jsondom.formData(),function(d){
										$.gmMessage("保存成功",true);
										...
										$(".saveBtn").validationEngine('showPrompt',"保存失败，请稍后重试",'error',null,true);
									}).always(function(){
										// mask
										$(".mask").hide();
										$(".loading-wrap").hide();
