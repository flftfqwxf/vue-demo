/*global plupload */
/*global qiniu */
function FileProgress(file, vm) {
    this.fileProgressID = file.id;
    this.file = file;
    this.opacity = 100;
    this.height = 0;
    this.fileProgressWrapper = $('#' + this.fileProgressID);
    var file = vm.$get('files.' + file.id)
    if (!file) {
        this.fileSize = plupload.formatSize(file.size).toUpperCase();
    } else {
        this.reset();
    }
...
FileProgress.prototype.setProgress = function (percentage, speed, chunk_size) {
    this.class = "progressContainer green";
    var file = this.file;
    var uploaded = file.loaded;
    var size = plupload.formatSize(uploaded).toUpperCase();
    var formatSpeed = plupload.formatSize(speed).toUpperCase();
    var progressbar = this.progressClass.find('td .progress').find('.progress-bar-info');
    if (this.statusText === '取消上传') {
        return;
    }
    this.statusText="已上传: " + size + " 上传速度： " + formatSpeed;
