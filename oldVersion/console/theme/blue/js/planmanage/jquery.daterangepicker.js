		...
		function init_datepicker()
		{
			...
			box.find('.apply-btn').click(function()
			{
				if(typeof opt.callback === 'function'){
					opt.callback.call(self, getDateString(new Date(opt.start)), opt.end ? getDateString(new Date(opt.end)) : '');
				}
				closeDatePicker();
			...
			if (defaults && defaults.length)
			{
				...
				if(defaults.length >= 2){
					...
					if(opt.singleDate){
						...
						setDateRange(moment(defaults[0], ___format, moment.locale(opt.language)).toDate(),moment(defaults[0], ___format, moment.locale(opt.language)).toDate());
					}
				}
