...
//				var d = new Date(dtArr[0], dtArr[1], dtArr[2]);
		...
		_defaultDates: function(options, obj, thisDate){
			...
			var lastDay = this._getMaxDate(year, month);console.log(month);
				...
				for(var i = 0; i < defData.length; i++){
					...
					if(home_year == this_year && home_month == this_month){
						...
						$("#"+this.options.dayTdIdFix+this_day).click(function(){
							...
							$(this).addClass("on");
							if(null != options.dayTdIdFixFun && 'function' == typeof options.dayTdIdFixFun){
						...
						if(thisDayData.minPrice && thisDayData.minPrice > 0 ){
							$("#"+this.options.dayTdIdFix+this_day).attr("price_list_index", i).addClass("set").append('<p class="prices">¥'+thisDayData.minPrice+'</p>');
						}
					}
				}
			}
		...
		_getLatelyDay : function(options){
			var defData = options.defaultData;
			var latelyDay = null;
			for(var i = 0; i < defData.length; i++){console.log(defData[i].date+"---"+options.nowDate);
				if(this._compareDay(defData[i].date, options.nowDate) >= 0){
					latelyDay = defData[i].date;break;
				}
			}
			return null != latelyDay ? new Date(latelyDay.replace(/-/g, "/")) : options.nowDate;
		},
		_compareDay : function(day, confimDay){
			var dayOne = new Date((this._formatDate(day, "yyyy-MM-dd") + " 00:00:00").replace(/-/g,"/"));
			var dayConfim = new Date((this._formatDate(confimDay, "yyyy-MM-dd") + " 00:00:00").replace(/-/g,"/"));
