...
$.Autocompleter.defaults = {
    ...
    showOthers:false,
    requestParamName:"q",
    othersOnClick:function (){return false},
...
$.Autocompleter.Select = function (options, input, select, config) {
	...
	function initPagination(){
		...
		pagination && orgData.pagination && pagination.empty().pagination({
	...
	function showNoData(){
		list.empty();
		var autocomplete_no_data = list.parent().find(".autocomplete_no_data");
		if(autocomplete_no_data && null != autocomplete_no_data && autocomplete_no_data.length > 0){
			autocomplete_no_data.show();
		}else{
			var noData = '<div class="autocomplete_no_data"><div class="no_data_title">无结果</div><div class="no_data_remark">请<span onclick="showLinkGmService(9999);" class="major_a" style="float:none;cursor:pointer;">联系客服</span>，我们会尽快添加</div></div>';
			var li = $(noData).appendTo(list.parent())[0];
			$.data(li, "ac_data", {});
		}
	}
	function hideNoData(){
		list.parent().find(".autocomplete_no_data").hide();
	}
	return {
