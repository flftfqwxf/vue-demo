...
function simpleUpload(obj, callback, url, maxFileSize) {
	...
	function getUploadPlugin(){
		$.fn.dmUploader === undefined && getScript(WEB_STATIC_CONSOLE+"/common/plugins/dmuploader/dmuploader.min.js");
	}
}

/*多图上传*/
function multiUpload(id, callback, url, path, maxNum){
	var options = {toolbars:[['insertimage']], serverUrl: "/gallery.json", areaPath: path ? path : '',imageMaxNum:maxNum||20};
...
function initUEditor(id, textlength, zIndex, touristLineId, maxScaleEnaledWidth, maxScaleEnaledHeight){
	var text = $("#"+id).text();
	var recentlyuse_url = $("#"+id).attr("recentlyuse");
	if(!textlength){
		textlength = $("#"+id).attr("maxlength");
	}
...
function initKindEditor(obj, textlength, zIndex, touristLineId, maxScaleEnaledWidth, maxScaleEnaledHeight,afterChange){
	...
	var kindeditor = KindEditor.create(obj, {
	    ...
	    afterChange : afterChange || function(){
	    	var statusBar = this.statusbar.get();
	...
	obj.focus(function(){
    	...
    	var mapDialog = $.dialog({
            ...
            content : '<div style="width:580px;height:400px;padding-bottom:10px;"><input id="set-search-value" type="hidden" value="'+v+'"/>'+
            		'<iframe id="map_iframe" class="%%-iframe" height="100%" width="100%" frameborder="0" src=WEB_STATIC_CONSOLE+"/common/plugins/map/mapcoordinate.html"></iframe></div>',
            ok: function () {
