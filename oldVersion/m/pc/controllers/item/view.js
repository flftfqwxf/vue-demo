...
function(controller) {
    ...
    controller.modules = {
        ...
        orderCheckout:function(){
            var adultNum=$("[name=adultNum]:first").val(),
        ...
        orderHandler:function(options){
        	...
        	if(v.type.indexOf("成人")>=0){
        	    adult.attr("data-id",v.id).attr("data-type",v.type).attr("price",v.price).text("￥"+v.price+"/人");
        	}
            ...
            function(serverData) {
                ...
                $("[name=tempDate]").eDate({
                    ...
                    drawCompate: function() {
                        ...
                        $(prices).each(function(i, v) {
                        	...
                        	datePrices.push({id:vvv.id,type:vvv.type,price:vvv.price});
                ...
                $("#J-nCalendar").eCalendar({
                    dataHandler: function() {
                        ...
                        //显示价格
                        var date, pricesDetail, dom, today = $(".fc-today:last").attr("data-date"),datePrices;
                        $(prices).each(function(i, v) {
                            date = v.date.replace(" 00:00:00", "");
                            datePrices=[];
                            pricesDetail = $(v.dateGroupPrices[0].prices).map(function(iii, vvv) {
        ...
        setbannerUrl:function(){
            //为特殊产品设置旅游攻略链接
        	var loct='http://'+location.hostname+"/news/579",this_pro=$(".installment").attr('pro-id'),price=$(".installment").attr("item-price");
        	var products={};
        	var html='';
        	if(products[this_pro]){
        		html='<a href="'+loct+'" target="_blank">'
        	        +'<div class="installment_info">'
        	        +'<img class="imgInst" src="http://static.gmmtour.com/m/pc/skin/images/gm-inst.png">'
        	        +'<span class="installment_price">￥<small class="font14">'+price+'</small>起+</span>'
        	        +'<span><small class="font14">'+price+'*11</small>期免息</span>'
        	        +'<button class="inst_btn">查看办理规则</button>'
        	        +'</div>'
        ...
        event: function() {
            ...
            fixedFormTop = fixedForm.offset().top,
                        ...
                        $("#J-hotel-scenic-dialog").window({
                            ...
                            event: function(dialog) {
                                ...
                                if (images.length > 0) {
                                    ...
                                    // $(".scenic-pic",dialog).html("<img src=\""+images[0].url+"@210w_120h_1e\"/>");
                                    // $('.scenic-ul li:eq(0)',dialog).addClass('selected');
