...
$(function(){
	...
	//提示订单与库存即将上线
	$(".disabled").mousemove(function(){
		if($(this).position()){
			$(".tips-disabled").css({left:$(this).position().left-20,top:($(this).position().top+$(this).height()-10)});
		}
		$(".tips-disabled").show();
	}).mouseout(function(){
		$(".tips-disabled").hide();
	});
	$(".no-access").mousemove(function(){
		if($(this).position()){
			$(".tips-no-access").css({left:$(this).position().left-20,top:($(this).position().top+$(this).height()-10)});
		}
		$(".tips-no-access").show();
	}).mouseout(function(){
		$(".tips-no-access").hide();
	...
	/*港马二维码*/
	$(document).on("mouseenter" , ".linkGmQrcode", function(){
	...
	$(document).on("mouseleave",".linkGmQrcode",function(){
		$(".weixinImg").remove();
...
//获取新申请员工数量
function getPendingStaffCount(){
	$.ajax({
		url:'/supplier/staff/pending-count',
		type: "GET",
		async:false,
		dataType: "json",
		...
		success:function(json){
			if (json.count) {
				$('#pendingStaffCount').text(json.count);
				$('#pendingStaffCount').attr("data-original-title", json.count + "个员工加入申请等待处理");
				$('#pendingStaffCount').show();
			} else {
				$('#pendingStaffCount').hide();
				$('#pendingStaffCount').text('');
			}
		}
	});
}
...
function getTodayOrderCount(){
	...
	$.ajax({
		...
		success:function(json){
			...
			if (count) {
                if(count.today){
                   $(".orderTotal:eq(0)").text(count.today).show();
                }else{
                   $(".orderTotal:eq(0)").hide();
                }
                if(count.dtoday){
                    $('#dtodayCount').text(count.dtoday).show();
                }else{
                    $('#dtodayCount').text('').hide();
                }

                if(count.stoday){
                    $('#stodayCount').text(count.stoday).show();
                }else{
                    $('#stodayCount').text('').hide();
                }
			}
		},
		error: function(){
            $('#dtodayCount, stodayCount').hide().text('');
		}
	});
}

//获取过期计划数量
function getExpiredTourPlanCount(){
	$.ajax({
		url:'/product/expired-count',
		type: "GET",
		async:false,
		dataType: "json",
		data: {format:'json',t:Date.now()},
		success:function(json){
			if (json.count) {
				$('#expiredTourPlanCount').text(json.count);
				$('#expiredTourPlanCount').attr("data-original-title", json.count + "个产品过期，请及时处理");
				$('#expiredTourPlanCount').show();
			} else {
				$('#expiredTourPlanCount').hide();
				$('#expiredTourPlanCount').text('');
			}
		}
	});
}
