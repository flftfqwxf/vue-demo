...
var artDialog = function (config, ok, cancel) {
	...
	artDialog.firstZindex = config.zIndex;
	_count ++;
...
artDialog.defaults = {
	...
	left: '50%',				// X轴坐标
	top: '38.2%',				// Y轴坐标
	zIndex: 91,				// 对话框叠加高度值(重要：此值不能超过浏览器最大限制)
...
$.confirm = $.dialog.confirm = artDialog.confirm = function (content, title, ok, cancel, zIndex) {
	...
	confirm = $.dialog({
        ...
        zIndex: zIndex?zIndex:defaultIndex+1
    });
	return confirm;
};
 ...
 */
$.prompt = $.dialog.prompt = artDialog.prompt = function (content, title, ok, defaultValue, placeholder) {
        ...
        content: [
            ...
            '<div>',
                '<input type="text" class="d-input-text" value="',
                    defaultValue,
                '" placeholder="'+placeholder+'" style="width:18em;padding:6px 4px" />',
            '</div>'
            ].join(''),
        init: function () {
