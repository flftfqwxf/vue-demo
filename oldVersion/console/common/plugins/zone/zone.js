...
//    	console.log("12345".substr(-1,2))

        var targetObj = $(this);
        var textContainer = $(arg.textContainer);
        var valContainer = arg.valContainer ? arg.valContainer : $("#areaId");
        var defaultArae;
...
//        console.log(totalLength)
        ...
        var initProvince = function () {
            ...
            for (j in arrByFirstLetter) {
                ...
                for (var k = 0; k < arrByFirstLetter[j].length; k++) {
                	for(var m = 0; m < arrByFirstLetter[j][k].childrens.length ; m++){
                		if(arrByFirstLetter[j][k].childrens[m].id == valContainer.val()*1) {
                			defaultArae = {};
                    		defaultArae.id = arrByFirstLetter[j][k].childrens[m].id;
                    		defaultArae.name = arrByFirstLetter[j][k].cn_name + arrByFirstLetter[j][k].childrens[m].cn_name;
                    	}
                	}
            ...
            //成都id2294
            valContainer && valContainer.val((defaultArae ? defaultArae.id : 2294));
            
            //初始化文本框内容
            textContainer.text(defaultArae ? defaultArae.name : (arg.defaultText?arg.defaultText:""));
        }
        ...
        targetObj.on("click",targetObj,function (e) {
            e.stopPropagation();
            if (!isClicked) {
            	if (!$(".select-zone")[0] || null == $(".select-zone")[0]) {
            		$(htmlHead + proDom + htmlFooter).appendTo(targetObj);
            	}
                isClicked = true;
            }
            $(".select-zone").css("display", "block");
            arg.zIndex && $(".select-zone").css("z-index", arg.zIndex);
            isClicked = false;
        })
