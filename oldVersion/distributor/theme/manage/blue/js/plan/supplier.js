...
$(function(){
	var supplierDialog = function(options) {
		var api, wrap, defaults = {
		        ...
		        init: function(here){
		            ...
		            wrap.find("#background .select-group label").removeClass("active").eq(0).addClass("active");
		        },
		        close: function(){
	...
	function saveUploadImage() {
		...
		submitForm(url +"/background.jsonp?image="+ $imgObj.attr("data-src"), "jsonp", null, function(d){
			$(".supplier-background").css({'background-image':'url("'+d.supplier.background+qa+'")'});
			oldType = $(":input[type='radio']:checked").val();
			$("#background .imagebar li.selected").removeClass("selected");
			$("#background .imagebar li.old-selected").removeClass("old-selected");
		});
	...
	$(document).on("click", ".nav-tabs > li", function() {
		...
		if(lineId) {
			$('tr[class*="plan_item_"]').hide();
			$('.plan_item_' + lineId).show();
		} else {
			$('tr[class*="plan_item_"]').show();
		}
	}).on("click", ".j_chang-box", function(){
			...
			button: [
				{
					name: "取消",
					callback: function() {
						var bar = $(".radio-group .active").find("input[type=radio]").val();
						if (bar == "uploadbar") {
	...
	function imgWidth(){
		var _width = screen.width,
			text = '\@'+_width+'w\_500h\_1e\_1c';
		var bg=$(".supplier-background").attr("bg");
		if(bg){
			bg=bg.replace("@200h_1920w_2e",text);
		}
		bg="background:#fafbfd url("+bg+");";
		$(".supplier-background").attr("style",bg);
	}
	imgWidth();