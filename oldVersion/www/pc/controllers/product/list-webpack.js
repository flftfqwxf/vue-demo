...
var listModule = {
    staticVars: function () {
        this.staticVars = {
            ...
            page: $(".list-page"),
            dataTimeTpl: "<div class=\"form-group  has-feedback\">" +
            "<input readonly type=\"text\" class=\"form-control input-sm hasDatepicker\" id=\"J-startDate\" onfocus=\"WdatePicker({minDate:'%y-%M-{%d}',maxDate:'#F{$dp.$D(\\'J-endDate\\')}'})\" name=\"startDate\" placeholder=\"最早出发\">" +
            "<span class=\"gmIcon gm-calendar form-control-feedback\"></span>" +
            "</div>" +
            "<div class=\"date_line\">-</div>" +
            "<div class=\"form-group  has-feedback\">" +
            "<input readonly type=\"text\" class=\"form-control input-sm hasDatepicker\" id=\"J-endDate\" onfocus=\"WdatePicker({minDate:'#F{$dp.$D(\\'J-startDate\\')||\\'%y-%M-{%d}\\'}'})\" name=\"endDate\" placeholder=\"最完出发\">" +
            "<span class=\"gmIcon gm-calendar form-control-feedback\"></span>" +
            "</div>" +
            "<div class=\"date_btn\">" +
            "<button class=\"btn btn-info btn-sm J-date-btn fl\" type=\"button\">确定</button>" +
            "</div>",
            pageTpl: "<div class=\"paginationWrap\">" +
            "<div>当前<span v=\"current\">1/1</span>每页<span v=\"per\">20</span>条 共<span v=\"total\">1</span>页</div>" +
            "<ul class=\"pagination\">" +
            "<li action=\"first\" page=\"1\"><a>&lt;&lt;</a></li>" +
            "<li action=\"prev\"><a>&lt;</a></li>" +
            "<li action=\"next\"><a>&gt;</a></li>" +
            "<li action=\"last\"><a>&gt;&gt;</a></li>" +
            "</ul>" +
            "</div>"
        }
    },
    ...
    listSearchEvent: function () {
        var that = this,
            ...
            name,
            value;
        vars.searchBox.on("click", "a", function () {
            ...
            query = {};
            query["startDate"] = $("#J-startDate").val();
            query["endDate"] = $("#J-endDate").val();
    ...
    listBuilder: function (data, query) {
        var that = this,
            vars = that.staticVars,
            library = {"departureTimes": "出发时间", "suppliers": "供应商", "tripDays": "行程天数", "types": "类型"},
            typeText = {"groupTour": "跟团游", "shipTour": "游轮", "freeWalkerTour": "自由行"},
            listBox = "",
            tbody = $("<div></div>"),
            tr = "",
            tourType = "",
            ...
            searchItemHtml = "",
            listContent = "",
            searchName = {"departureTimes": "出发时间", "suppliers": "供应商", "tripDays": "行程天数", "types": "类型"};
        //产品数量显示
        ...
        else if (parseInt(query.page) == 1 && ((!query.minPrice) && (!query.maxPrice) && (!query.airlineCode))) {
            vars.searchBox.hide();
        }
        //有产品时
        if (data.productList.length > 0) {
            $(data.productList).each(function (i, v) {
                ...
                $("[v=brightPoints]", tr).text(v.brightSpotNotTags).attr("title", v.brightSpotNotTags);
        ...
        $(data).each(function (i, v) {
            ...
            if (type == "groupTour") {
                ...
                $(v.prices).each(function (ii, vv) {
                    ...
                    if (vv.remark) {
                        $("[v=remark]", item).append("，" + vv.remark);
                    }
                    price.append(item);
                });
                recommend.text("在上述同行价格基础上加" + v.extraPrice + "元");
    ...
    listRequest: function (data) {
        ...
        data["minPrice"] = $("[name=minPrice]").val();
        data["maxPrice"] = $("[name=maxPrice]").val();
        data["airlineCode"] = $("[name=airlineCode]").val();
        //search box input
    ...
    searchClick: function (value, supplier) {
        ...
        if (supplier) {
            this.listRequest({supplier: supplier});
        }
        else {
            this.listRequest({keyWord: value});
        }
    },
        ...
        if ((!$.cookie("id")) && (!$.cookie("name"))) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    else i.listRequest({specification: 0});
                }
            });
        }
        else {
    ...
    event: function () {
        ...
        //事件代理
        $(document).on("click", ".J-city-button, .J-search-button", function () {
        ...
        }).on("click", ".J-city-list a,.J-search-list a", function () {
            //two for city search
            ...
            $.cookie("name", _name);
            i.ipSearch();
        }).on('click', '.J-gm-close', function () {
            //gm-close icon state
            $(".J-search-txt").val("");
            $(this).hide();
            $(".J-fuzzy-find-list").hide();
        }).on('click', '.J-city-con-right a', function () {
            //sub search-list a click
            var _value = $(this).text();
            i.searchClick(_value);
            $(".J-hot-city-list").hide();
        }).on('click', '.J-search-city-ul a', function (event) {
            //left city a click
            var _value = $(this).text();
            i.searchClick(_value);
        ...
        }).on('click', '.J-fuzzy-find-list li', function (event) {
            //fuzzy search click
        ...
        }).on("mouseenter", ".J-search-city-list", function () {
            //left city list meanu
            $(".J-new-ul").show().addClass('new');
        }).on("mouseleave", ".J-search-city-list", function () {
            //left city list meanu
            $(".J-new-ul").hide().removeClass("new");
        }).on("click", "#hot_tourist_line .hot_li_title", function () {
            //hot supplier click
            ...
            if (!isNaN(page)) {
                i.listRequest({page: page});
            }
        }).on("mouseenter", "#J-top-slider li", function () {
            $(this).find(".text_info_bg").stop(true).animate({height: 60}, 500);
            $(this).find(".text_info").stop(true).animate({height: 60}, 500);
        }).on("mouseleave", "#J-top-slider li", function () {
            $(this).find(".text_info_bg").stop(true).animate({height: 30}, 500);
            $(this).find(".text_info").stop(true).animate({height: 30}, 500);
            ...
            i.listRequest({"sort": $(this).attr("data-id")});
            ...
            if (minPrice || maxPrice) {
                if ((!isNaN(minPrice)) && (!isNaN(maxPrice)) && (parseInt(minPrice) > parseInt(maxPrice))) {
                    $.showMsg("最低价不能高于最高价。");
                    $("[name=minPrice], [name=maxPrice]").val("");
                    return false;
                }
                i.listRequest({"specification": 0, "minPrice": minPrice, "maxPrice": maxPrice});
                $(this).hide().parent().after("<a class=\"clear\">清除</a>");
            }
        }).on("click", "#J-airplane-search", function () {
            var airline = $("[name=airlineCode]").val();
            if (airline) {
                i.listRequest({"specification": 0, "airlineCode": airline});
                $(this).hide().parent().after("<a class=\"clear\">清除</a>");
            }
