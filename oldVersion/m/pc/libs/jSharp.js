/*!
 * jSharp v1.0 lavenler@live.cn
 * Copyright 2015 skiny
 * Version 1.0
 */
(function(){
	...
	//require js 2.1.22
	var requirejs,require,define,basePath,configJs,globalJs,jSharpJs="jSharp.js",getScriptSrc=function(keyword){var result="";for(var i=0;i<document.scripts.length;i++){result=document.scripts[i].getAttribute("src");if(result&&result.toLowerCase().indexOf(keyword.toLowerCase())>=0){break}}return result};
	(function(ha){var basePath=getScriptSrc(jSharpJs);basePath=basePath.substring(0,basePath.lastIndexOf("/"));configJs=basePath+"/config.js";globalJs=basePath.replace("libs","controllers")+"/global.js";function L(b){return"[object Function]"===R.call(b)}function M(b){return"[object Array]"===R.call(b)}function x(b,c){if(b){var d;for(d=0;d<b.length&&(!b[d]||!c(b[d],d,b));d+=1);}}function Y(b,c){if(b){var d;for(d=b.length-1;-1<d&&(!b[d]||!c(b[d],d,b));--d);}}function w(b,c){return la.call(b,c)}function g(b,c){return w(b,c)&&b[c]}function E(b,c){for(var d in b)if(w(b,d)&&c(b[d],d))break}function Z(b,c,d,k){c&&E(c,function(c,g){if(d||!w(b,g))!k||"object"!==typeof c||!c||M(c)||L(c)||c instanceof
...
c=(1===d.length?["require"]:["require","exports","module"]).concat(c)));T&&(g=Q||pa())&&(b||(b=g.getAttribute("data-requiremodule")),e=K[g.getAttribute("data-requirecontext")]);e?(e.defQueue.push([b,c,d]),e.defQueueMap[b]=!0):W.push([b,c,d])};define.amd={jQuery:!0};k.exec=function(b){return eval(b)};k(v)}})(this);
	...
	//load config and global controller
	configJs&&globalJs&&require([configJs,globalJs]);
	
	//jSharp 
	var jSharp=window.jSharp={};
	
	//jSharp src
	jSharp.getSrc=getScriptSrc;
	
	//jSharp version
	jSharp.getVersion=function(src){
		src=src||jSharp.getSrc(jSharpJs);
		var keyword="version=";
		return src.substring(src.lastIndexOf(keyword)+keyword.length,src.length)||"";
	};
	
	//jSharp controller
	jSharp.controller=function(){
		this.requires=[];
		this.modules={};
	};
	jSharp.controller.prototype={
		using:function(requires){
			typeof requires=="string"&&this.requires.push(requires);
		...
		call:function(){
			var that=this;
			jSharp.using(this.requires,function(){
				typeof that.modules.init=="function"&&that.modules.init();
			});
	...
	//jSharp config
	jSharp.config=function(options){
	        var temp,
        	    jSharpSrc=jSharp.getSrc(jSharpJs);
        	
        	if(jSharpSrc.indexOf("/dist/")>=0){
        	    var base=jSharpSrc.substring(0,jSharpSrc.indexOf("dist")),
        	    	path=jSharpSrc.substring(jSharpSrc.indexOf("dist"),jSharpSrc.length).split("/");
        	    options.baseUrl=base+path[0]+"/"+path[1]+"/";
        	}
        	require.config(options);
	};
	
	//jSharp using
	window.using=jSharp.using=function(modules,callBack){
		require([configJs,globalJs],function(){
	...
	//jSharp define
 	window.define=jSharp.define=function(modules,callBack){
