;( function( $, window, undefined ) {
	...
	$.DatePrice.defaults = {
		...
		maxYear:2099,
		showMore:false,
		preDataDisabled:false,//今天以前的日期是否可点击，false：不能点击
		dateFmt:"yyyy-MM-dd",
		...
		_setDateObj : function(options){
			...
			var objHtml = '<div class="gm_date_price">\n\t'+
							'<div class="gm_date_price_title">\n\t\t'+
								'<div class="gm_date_price_prov"><i class="gmIcon gm-back"></i></div>\n\t\t'+
								'<div class="gm_date_price_next"><i class="gmIcon gm-next"></i></div>\n\t<span id="gm_date_price_show_year">'+nowYear+"年"+nowMonth+"月</span>"+
...
//				var d = new Date(dtArr[0], dtArr[1], dtArr[2]);
			...
			$("#"+this.options.objId).find(".gm_date_price_next").click(function(){
				...
				maxMonth,maxYear,maxDate=s_this.options.maxDate;
				if(maxDate){
					maxMonth=maxDate.getMonth(),
					maxYear=maxDate.getFullYear();
				}
				if(maxDate){
					if(this_D.getFullYear() == maxYear &&  this_D.getMonth() <= maxMonth){//年份等于，比较月份大小
						s_this._defaultDates(s_this.options, obj, this_D);
					}
					if(this_D.getFullYear() < maxYear){//当前显示的年份小于最大发团日期
						s_this._defaultDates(s_this.options, obj, this_D);
					}
					else{
						return;
		...
		_defaultDates: function(options, obj, thisDate){
			...
			for(var i = fDay; i <= lastDay; i++){
				...
				if(options.preDataDisabled && now > Date.parse(new Date(year,(month*1-1),i+1))){
					disaabledStr='greyDisabled';
				}else{
					disaabledStr='';
				}
				str += '<td><div class="td_content '+disaabledStr+'" id="'+this.options.dayTdIdFix+i+'"><p class="dayNumber" >'+i+'</p>';		
				str += '</div></td>';
				if(n%7 == 0){
					str += '</tr>';
				}
			...
			this.options.initHandle && this.options.initHandle();
		},
		setDayPrice : function(options, homeDate){
			...
			var home_day=homeDate.getDate();
			if(defData.length > 0 && !options.defaultClick){
				for(var i = 0; i < defData.length; i++){
					...
					if(home_year == this_year && home_month == this_month){
						...
						$("#"+this.options.dayTdIdFix+this_day).click(function(){
							var has=$(this).hasClass("greyDisabled");
							if(has){
								return ;
							}
						...
						if(now >= thisDayData.date ){
							$("#"+this.options.dayTdIdFix+this_day).attr("price_list_index", i).append('<p class="prices">¥'+thisDayData.minPrice+'</p>');
						}else{
							$("#"+this.options.dayTdIdFix+this_day).attr("price_list_index", i).addClass("set").addClass(sale).attr("date-value",thisDayData.date).append('<p class="prices">¥'+thisDayData.minPrice+'</p>');
						}
					}
				}
			} else {
				$(".td_content").click(function(){
					...
					options.dayTdIdFixFun && options.dayTdIdFixFun($(this), home_year + "-" + home_month + "-" + day);
				});
			}
		},
...
//		    });
		...
		_getProvMonthYestdy : function(date){
			...
			var mon = parseInt(strMonth,10);
			var strDay= new Date(strYear, mon, 0);
