	...
	controller.modules={
			...
			$("#J-dates .new_form_control").each(function(i){
				...
				$(this).eCalendar({
					...
					disabled: function (date) {
						var day=eCalendar.moment(date).format('t_YYYYMMDD');
						return (days[day]?0:1);
					},
					onSelected:function(date){
						var date=date.replace("年","-").replace("月","-").replace("日",""),
						day=eCalendar.moment(date).format('t_YYYYMMDD');
						$("input",dates.eq(i)).attr("value",eCalendar.moment(date).format('YYYY年MM月DD日'));
		...
		event:function(){
			...
			$(document).on("click",".J-left-btn",function(){
				/*左移的点击事件*/
				var _num = $(this).index('.J-left-btn');
				var htm1 = com.eq(_num).html();
				var htm2 = com.eq(_num-1).html();
				if (_num == 0) {
					return false;	
				}else{
					table.each(function(index, el) {
						$('tr',$(this)).each(function(index, el) {
							var tds = $('td',$(this));
							var temp1 = tds.eq(_num).html();
							var temp2 = tds.eq(_num-1).html();
							tds.eq(_num).html(temp2);
							tds.eq(_num-1).html(temp1);
						});
					});
					com.eq(_num).html(htm2);
					com.eq(_num-1).html(htm1);
					i.status();
			...
			}).on('click', '.J-right-btn', function() {
				var num = $(this).index('.J-right-btn');
				...
				/*$(".comparison>div").each(function(){
					if($(this).hasClass("fixed_box")) return;
					var top = $(this).offset().top
					console.log($(this).attr("class"),$(this).offset().top);
					if(top<100&&hei <= end - 100){
						console.log(true);
					} else {
						console.log(false);
					}
