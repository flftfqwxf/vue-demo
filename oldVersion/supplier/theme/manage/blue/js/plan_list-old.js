...
$(function() {
    ...
    $(".mod-select").mgSelect();
    var bar = $("#tabmenu").height(), top = $(".top_info").height(), windowHeight = $(window).height(),
        documentHeight = $(document).outerWidth(true), h = bar + top + 36;
    var scrolls = function(){
        ...
        if ($(window).scrollTop() <= h) {
            $("#tabmenu").removeClass("bar-scroll");
            $("#msg").css({
                "position" : "absolute",
                "top" : "200px"
            });
    ...
    // 出团计划弹框.
        ...
        $d_price = $("#dialog_price"), $d_price_c = $d_price.find(".content"), c = "selected",
        $planDialog, u = Plan.u, $form, delArr = [], _title;

    $(document).on("click", ".tcpage-content .side-l a", function(e) {
        $(this).siblings("." + c).removeClass(c);
        $(this).addClass(c);
        //$planDialog.find(".main").mCustomScrollbar("scrollTo", $("#" + $(this).attr("href").substring(1)));
        $planDialog.find(".main").off("scroll", scolls);
    }).on("mouseout", ".tcpage-content .side-l a", function(e) {
        $planDialog.find(".main").on("scroll", scolls);
        ...
        planDialog($(template("planDialog", {lineId : lineId})), {lineId : lineId});
        ...
        $.getJSON("/supplier/tour-plan/" + id + ".json?_=" + Math.random(), function(d) {
            ...
            planDialog($(template("planDialog", planData)), planData);
        })
        ...
        $.getJSON("/supplier/tour-plan/" + id + ".json?_=" + Math.random(), function(d) {
            ...
            planData["lineId"] = $(":input[name='lineId']").val();
            planDialog($(template("planDialog", planData)), planData);
            //$planDialog.find(".main").mCustomScrollbar("scrollTo", myDialog.DOM.wrap.find(".J_set_tour"));
            $planDialog.find(".main").scrollTo("#travel-set");
        });
    }).on("mouseover", ".advert", function() {
        showBannerImage($(this));
    }).on("mouseout", ".advert", function() {
        hideBannerImage($(this));
    }).on("mouseover", ".get-focus", function() {
        $(this).find(".del_plan").show();
    }).on("mouseout", ".get-focus", function() {
        $(this).find(".del_plan").hide();
    }).on("click", ".search-btn", function() {
        $(":input[name='lineId']").val("");
        $("#curentPage").val("1");
        params['page'] = 1;
        u.search();
    }).on("click", "#clearSearch", function() {
        $(":input[name='lineId']").val("");
        $(":input[name='name']").val("");
        $("#curentPage").val("1");
        params['page'] = 1;
        u.search();
    }).on("click", "#menu-line li", function() {
        if ($(this).parent().has("span").length) {
            $(this).closest("#menu-line").find("." + c).removeClass(c);
        } else {
            $("#menu-line").find("span." + c).removeClass(c)
            !$("#more-select").hasClass(c) && $("#more-select").addClass(c);
        }
        $(this).addClass(c);
        ...
        if(parent.hasClass("option-list")) {
            var datas = {
    ...
    }).on("click", ".add_group", addGroup).on("click", ".del_group", function() {
        ...
        var _this = $(this), data = [];
        $.confirm("确认要删除该计划吗？", "确认提示", function() {
    ...
    }).on("mouseover", "td[show-title]", function(d){
    ...
    function planDialog(dialogObj, planData) {
        ...
        myDialog = tourPlanDialog({
            ...
            init : function() {
                ...
                $d_price_c = $d_price.find(".content");
        ...
        myDialog.DOM.wrap.on("change", ":radio[name=type]", function(e, data) {
        ...
        }).on("click", ".buttonbar .cancel", function() {
        ...
        }).on("click", ".buttonbar .save", function() {
            if (!isValid()) {
                return false;
            }
            ...
            $(".group", $d_price_c).each(function(k, v) {
                var d = u.inputToJson($(".items", this)), dates = [], prices = [];
                if (type == "date") {
                    $(".cols3", this).find(".item").each(function() {
                ...
                var prices = [];
                $(".cols2", this).find("tr").each(function() {
                    var t = u.inputToJson($(this));
                    prices.push(t);
                });
                d.prices = prices;
            ...
            if (dataUrl == "/tour/input?id=") {
                ...
                dataUrl += myDialog.DOM.wrap.find("[name='touristLine.id']").val() + "&name=" + myDialog.DOM.wrap.find(".plan input[name='name']").val();
            } else {
    ...
    function updateScroll(){
        baseInfoTop = $("#base-info").offset().top - $planDialog.find(".main").offset().top;
        groupPriceTop = $("#group-price").offset().top - $planDialog.find(".main").offset().top;
        travelSetTop = $("#travel-set").offset().top - $planDialog.find(".main").offset().top;
        addSetTop = $("#add-set").offset().top - $planDialog.find(".main").offset().top;
    }
    // 渲染团期报价模板。
    function buildGroupContent(d) {
        ...
        //$planDialog.find(".main").mCustomScrollbar("update");
        return $new;
    }
    function addGroup(e, d) {
        ...
        var dateItemLen = $($d_price_c.find(".group")[$d_price_c.find(".group").length - 1]).find('.item').length || 0;
        if (dateItemLen < 1) {
        ...
        if ($d_price_c.find(".group").length == 10) {
            ...
            //$planDialog.find(".main").mCustomScrollbar("scrollTo", $new);
            travelSetTop += $new.height();
            addSetTop += $new.height();
            $planDialog.find(".main").scrollTo(travelSetTop-450);
        }
    }
...
(function($) {
    ...
    function Group(ctx, data) {
        ctx.on("click", ".add_priceitem", addPriceItem).on("click", ".price_table tr td .del_tr", function() {
        ...
        ctx.find("[name='airline.name']").autocomplete("/flight/airlines.json", {
            ...
            var $itemsObj = $(this).closest(".items");
        ...
        $datepicker.each(function(i) {
            ...
            if ($self.is("input")) {
                ...
                toDayShow($self, $dates.find(".item:eq(0)").attr("id") || new Date().format(), defaultValues);
            }
        });

        // 团期删除按钮绑定事件
        $dates.on("click", "i.del_date_item", function() {
            var dateItemLen = $dates.find('.item').length;
            if (dateItemLen <= 1) {
            ...
            $($datepicker.find("td[date-id='" + $(this).closest(".item").attr("id") + "']")).find("a." + c).removeClass(c);
